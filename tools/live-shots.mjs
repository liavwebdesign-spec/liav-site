// צילומי אימות לאתר החי ב-Lovable: הירו אחרי הכניסה, בדסקטופ ובנייד, עברית ואנגלית.
import { spawn } from "node:child_process";
import fs from "node:fs";
import path from "node:path";
const CH = "C:/Program Files/Google/Chrome/Application/chrome.exe";
const BASE = "https://liav-matzri.lovable.app";
const out = path.join(process.cwd(), "qa-shots", "live"); fs.mkdirSync(out, { recursive: true });
const port = 9930 + Math.floor(Math.random() * 40);
const chrome = spawn(CH, ["--headless=new", "--hide-scrollbars", `--remote-debugging-port=${port}`, `--user-data-dir=${path.join(process.env.TEMP, "live-" + port)}`, "--window-size=1440,900", "about:blank"], { stdio: "ignore" });
...
for (const [w, h, m, t] of [[1440, 900, false, "d"], [390, 844, true, "m"]]) {
  await view(w, h, m);
  for (const [route, name] of [["/", "he"], ["/en", "en"]]) {
    await send("Page.navigate", { url: BASE + route }); await sleep(8000); await shot(`${t}-${name}-hero`);
  }
}
